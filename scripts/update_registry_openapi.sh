...
GOOD_URL="https://${SERVICE}-${PROJECT_NUMBER}.${REGION}.run.app"
REV="$(gcloud run services describe "$SERVICE" --project="$PROJECT_ID" --region="$REGION" \
  --format='value(status.latestReadyRevisionName)')"

cp REGISTRY.md REGISTRY.md.bak.$(date +%Y%m%d-%H%M%S) 2>/dev/null || true

# 1) Eliminar cualquier bloque previo de OpenAPI (por emoji o tag)
perl -0777 -pe 's/\n*---\n+### (?:\xF0\x9F\xA7\xA9|<0001f9e9>) OpenAPI Schema .*?(?=\n---\n|\Z)//gms' -i REGISTRY.md

# 2) Compactar separadores/líneas en blanco
perl -0777 -pe 's/\n{3,}/\n\n/g; s/(?:\n---\n){2,}/\n---\n/g' -i REGISTRY.md

# 3) Escribir bloque limpio
cat >> REGISTRY.md <<EOF
