...
# === NODOS PRINCIPALES ===
net.add_node("Infraestructura", shape="ellipse", color="#E67E22", size=40, title="Centro de control general")

# Docker
net.add_node("Docker", shape="box", color="#3498DB", title="Infraestructura local (contenedores)")
net.add_edge("Infraestructura", "Docker")

# Cloud
net.add_node("Cloud Run", shape="box", color="#2ECC71", title="Servicios desplegados en Google Cloud Run")
net.add_edge("Infraestructura", "Cloud Run")

# Firestore
net.add_node("Firestore", shape="box", color="#F39C12", title="Base de datos en la nube")
net.add_edge("Infraestructura", "Firestore")

# FHIR
net.add_node("FHIR", shape="box", color="#9B59B6", title="Almacenamiento de datos clínicos")
net.add_edge("Infraestructura", "FHIR")

# Integraciones
net.add_node("Integraciones", shape="box", color="#1ABC9C", title="Servicios externos conectados")
...
try:
    for c in containers:
        ...
        net.add_node(c.name, shape="dot", color=color, title=f"Contenedor {c.name} ({c.status})")
...
try:
    for s in services:
        ...
        net.add_node(name, shape="dot", color="#27AE60", title=f"Servicio Cloud Run\n{url}")
