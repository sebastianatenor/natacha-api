...
# 🔹 Importamos funciones del historial local
from health_monitor.infra_local_history import (
    HISTORY_FILE,
    get_history,
    clear_history
)
...
def push_to_firestore(entry: dict):
    """Sube una entrada única a Firestore."""
...
def sync_local_to_firestore():
    ...
    - Si hay diagnósticos locales pendientes, los sube.
    ...
    try:
        ...
        clear_history()
        return {"status": "ok", "synced": synced, "detail": "Sincronización completa"}
...
def pull_from_firestore(limit: int = 20):
    """Obtiene los últimos registros de Firestore."""
    try:
        client = get_firestore_client()
        docs = (
