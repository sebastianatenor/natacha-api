...
def get_firestore_client():
    """Obtiene un cliente de Firestore con autenticación automática."""
    return firestore.Client()


def restart_service(service_name):
    """Reinicia un servicio Cloud Run moviendo el tráfico a la última revisión."""
    try:
        subprocess.run(
            [
        ...
        print(f"⚠️ Error al reiniciar {service_name}: {e.stderr}")
        return {"service": service_name, "status": "error", "detail": e.stderr}
    except Exception as e:
        print(f"⚠️ Error inesperado al reiniciar {service_name}: {e}")
...
def log_to_firestore(entry):
    ...
    try:
        ...
        entry = {
            **entry,
            "timestamp": ts,
            "ts_server": firestore.SERVER_TIMESTAMP,  # útil para queries por hora servidor
        }
        ...
        print(f"🧠 Evento registrado en Firestore: {doc_id}")
    except Exception as e:
        print(f"⚠️ No se pudo registrar el evento en Firestore: {e}")


def auto_heal():
    """
    Escanea los servicios Cloud Run y reinicia los que estén en error.
    También guarda los resultados del proceso en Firestore.
    """
    print("🚀 Ejecutando Auto-Healer Natacha...")
    summary = {
    ...
    try:
        ...
        for s in services:
            ...
            if any(word in status for word in ["error", "failed", "crash", "timeout"]):
                print(f"🚨 Servicio en estado anómalo: {name} ({status})")
                ...
                print(f"✅ Servicio saludable: {name} ({status})")
        ...
        try:
            ...
            print(f"⚠️ No se pudo registrar el resumen en Firestore: {e}")

        print(f"📋 Resumen final registrado: {summary}")

    except Exception as e:
        print(f"❌ Error general en Auto-Healer: {e}")
...
if __name__ == "__main__":
    result = auto_heal()
    print("✅ Auto-Healer ejecutado manualmente:")
    print(result)
