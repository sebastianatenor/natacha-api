...
def ensure_secret(name):
    ...
    if not ok:
        ...
        run_cmd(f"gcloud secrets create {name} --replication-policy='automatic' --project=asistente-sebastian")
        return False
    return True

def ensure_scheduler_job(name, schedule, uri, service_account):
    ok, _ = run_cmd(f"gcloud scheduler jobs describe {name} --region=us-central1 --project=asistente-sebastian")
    if not ok:
        print(f"[CREATING SCHEDULER JOB] {name}")
        run_cmd(f"gcloud scheduler jobs create http {name} "
...
def ensure_pubsub_topic(topic):
    ok, _ = run_cmd(f"gcloud pubsub topics describe {topic} --project=asistente-sebastian")
    if not ok:
...
def ensure_run_service_account(service, account):
    ok, _ = run_cmd(f"gcloud run services describe {service} --region=us-central1 --project=asistente-sebastian | grep {account}")
    if not ok:
        print(f"[UPDATING SERVICE ACCOUNT] {service} -> {account}")
        run_cmd(f"gcloud run services update {service} "
