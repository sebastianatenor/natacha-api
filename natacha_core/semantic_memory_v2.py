...
OPENAI_CHAT_MODEL = "gpt-4o-mini"  # mismo modelo lógico que usabas antes
...
def _embed_text(text: str) -> List[float]:
    """
    Crea un embedding usando la API HTTP de OpenAI (sin SDK).
    """
    if len(text) > 8000:
        text = text[:8000]
...
def _chat_completion(prompt: str, max_tokens: int = 256) -> str:
    """
    Llama a la API de chat de OpenAI vía HTTP para generar el resumen.
    """
    payload = {
        "model": OPENAI_CHAT_MODEL,
        "messages": [
            {"role": "system", "content": "Eres memoria ejecutiva."},
...
def summarize(user_id=None, project=None, q=None, limit=5, max_tokens=256):
    ...
    try:
        summary_text = _chat_completion(prompt, max_tokens=max_tokens)
